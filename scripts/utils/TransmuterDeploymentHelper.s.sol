...
contract TransmuterDeploymentHelper is Utils {
    ...
    /// @dev Deploys facets, diamond and connects facets
    ...
    function _deployTransmuterWithoutFacets(
        address _init,
        bytes memory _calldata
    ) internal virtual returns (ITransmuter transmuter) {
        // Deploy every facet
        facetNames.push("DiamondCut");
        facetAddressList.push(DIAMOND_CUT_FACET);

        facetNames.push("DiamondEtherscan");
        facetAddressList.push(DIAMOND_ETHERSCAN_FACET);

        facetNames.push("DiamondLoupe");
        facetAddressList.push(DIAMOND_LOUPE_FACET);

        facetNames.push("Getters");
        facetAddressList.push(GETTERS_FACET);

        facetNames.push("RewardHandler");
        facetAddressList.push(REWARD_HANDLER_FACET);

        facetNames.push("SettersGovernor");
        facetAddressList.push(SETTERS_GOVERNOR_FACET);

        facetNames.push("SettersGuardian");
        facetAddressList.push(SETTERS_GUARDIAN_FACET);
